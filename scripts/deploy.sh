...
if [[ -n "$(git status --porcelain)" ]]; then
  git add content content-v1-backup theme api config.yaml config.dev.yaml config.regru.yaml rules.yaml scripts deploy .github README.md FRONTMATTER.md .gitignore media
