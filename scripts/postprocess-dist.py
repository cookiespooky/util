...
def close_links(text: str) -> str:
    ...
    #
    # Условие «без слэша на конце» задаётся последним классом символов, и это
    # существенно: в классе повторения слэш есть, поэтому выражение вида
    # [A-Za-z0-9_\-/]+ захватывало и уже закрытый адрес, дописывая ему второй
    # слэш. Так «/services/» превращалось в «/services//» — и попадало в том
    # числе в canonical, который layout.html отдаёт уже со слэшем.
    link = re.compile(
        r'(href=")(' + re.escape(base) + r'/[A-Za-z0-9_\-/]*[A-Za-z0-9_\-])(")'
    )
