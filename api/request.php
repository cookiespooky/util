...
/* Метку времени ставит v2.js по часам посетителя — на статическом сайте
   серверного времени в разметке взяться неоткуда. Отсюда два ограничения,
   и оба означают «пропустить», а не «отсеять»:

   отметки нет вовсе — это отправка без JavaScript, она поддерживается;

   отметка из будущего — часы посетителя спешат. Раньше такая заявка молча
   отбрасывалась с ответом «принято»: разность выходила отрицательной, то есть
   заведомо меньше минимума. Сбитые часы на телефоне не редкость, и человек
   при этом видел подтверждение отправки. Роботом считаем только правдоподобно
   быстрое заполнение, то есть неотрицательную разность меньше минимума. */
$startedAt = (int) $field('form_ts');
$minFill = (int) ($config['min_fill_sec'] ?? 3);
$filledIn = $startedAt > 0 ? time() - $startedAt : null;
if ($filledIn !== null && $filledIn >= 0 && $filledIn < $minFill) {
    json_response(['ok' => true]);
}
